...
def get_train():
    ...
    df_train_match.tourney_date = pd.to_datetime(df_train_match.tourney_date.apply(to_date))

    df_train = pd.merge(df_train_match, df_train_odds, left_on=['winner_name', 'loser_name', 'tourney_date'], right_on=['Winner', 'Loser', 'Date'])
    return df_train
...
def get_test():
    ...
    df_test_match.tourney_date = pd.to_datetime(df_test_match.tourney_date.apply(to_date))

    df_test = pd.merge(df_test_match, df_test_odds, left_on=['winner_name', 'loser_name', 'tourney_date'], right_on=['Winner', 'Loser', 'Date'])
    return df_test
